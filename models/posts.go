...
func (r *postR) GetRatings() RatingSlice {
	if r == nil {
		return nil
	}
	return r.Ratings
...
var (
	postAllColumns            = []string{"id", "title", "content", "user_id", "created_at", "updated_at"}
	postColumnsWithoutDefault = []string{"title"}
	postColumnsWithDefault    = []string{"id", "content", "user_id", "created_at", "updated_at"}
...
func (o *Post) Ratings(mods ...qm.QueryMod) ratingQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (postL) LoadRatings(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	var slice []*Post
	var object *Post
	...
	query := NewQuery(
		qm.From(`ratings`),
		qm.WhereIn(`ratings.post_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Post) SetComments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Comment) error {
	query := "update \"comments\" set \"post_id\" = null where \"post_id\" = $1"
...
// Sets related.R.Post appropriately.
func (o *Post) AddRatings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Rating) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.PostID, o.ID)
...
// post replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Post's Ratings accordingly.
// Replaces o.R.Ratings with related.
// Sets related.R.Post's Ratings accordingly.
func (o *Post) SetRatings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Rating) error {
	query := "update \"ratings\" set \"post_id\" = null where \"post_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
// Sets related.R.Post.
func (o *Post) RemoveRatings(ctx context.Context, exec boil.ContextExecutor, related ...*Rating) error {
	if len(related) == 0 {
		return nil
	}
